...
impl Page {
    ...
    pub async fn from_html_with_url(
        ...
        event_loop.run_until_idle().await?;
    ...
    pub async fn navigate_warm(&mut self, url: &str) -> Result<(), deno_core::error::AnyError> {
        ...
        // fetch and no refed render timer (readiness.rs).
        let _ = self.event_loop.run_until_settled().await;
    ...
    async fn navigate_loop_internal(
        ...
        for iter in start_iter..iterations {
            ...
            if let Ok(Err(e)) =
                tokio::time::timeout(drain_timeout, page.event_loop().run_until_idle()).await
            {
                    ...
                    || started_as_awswaf_challenge)
            {
                ...
                loop {
                    ...
                    let _ =
                        tokio::time::timeout(remaining, page.event_loop().run_until_idle()).await;
            ...
            let v8_refetched: Option<String> = if same_host_reload {
                ...
                if started_as_interstitial_challenge {
                    ...
                    let _ = tokio::time::timeout(
                        ...
                        page.event_loop().run_until_idle(),
                    )
                    .await;
                }
